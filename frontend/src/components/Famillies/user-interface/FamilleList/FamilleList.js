  ...
  return (
    <>
      <button
        ref={btnRef}
        onClick={toggle}
        data-testid={`famille-actions-${row.id}`}
        style={{
      ...
      {open && ReactDOM.createPortal(
        <div
          style={{
            ...
            boxShadow: '0 8px 24px rgba(15, 23, 42, 0.15)', zIndex: 10000,
            overflow: 'hidden',
          }}
...
const FamilleList = ({detailView}) => {
  ...
  const tabs = useMemo(() => (objectFamillesData || []).map((o) => ({
    uid: o.uid, label: o.label, code: o.name, typesId: o.typesId,
  })), [objectFamillesData])

  const activateTab = (tab) => {
    setLoading(true)
    dispatch(setSelectedObject({uid: tab.uid, name: tab.code, typeId: tab.typesId}))
    dispatch(fetchFamilles({src: tab.code})).finally(() => setLoading(false))
  }
  ...
  const getTabCode = () => selectedObject?.name || (tabs[0]?.code)
  const tabLabel = (code) => (objectFamillesData?.find((o) => o.name === code)?.label) || 'Tag'

  return (
    <div className='lt-page' data-testid='famille-list-page'>
      {/* ── Header ── */}
      <div style={{display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 16}}>
        <div>
          <h1 style={{margin: 0, fontSize: '1.6rem', fontWeight: 800, color: '#0F172A', fontFamily: "'Manrope', sans-serif", letterSpacing: '-0.02em'}}>Familles</h1>
          <div style={{display: 'flex', alignItems: 'center', gap: 6, marginTop: 6, fontSize: '0.78rem', color: '#94A3B8'}}>
            ...
            <span style={{color: '#475569', fontWeight: 600}}>Familles</span>
          </div>
        </div>
        <button
          data-testid='famille-create-btn'
          onClick={onCreate}
          style={{
            ...
            boxShadow: '0 6px 16px rgba(37, 99, 235, 0.35)', transition: 'all 0.18s',
          }}
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                data-testid={`famille-tab-${tab.code}`}
                disabled={loading}
                style={{
                  ...
                  cursor: loading ? 'wait' : 'pointer', fontWeight: 700, fontSize: '0.85rem',
          ...
          <table style={{width: '100%', borderCollapse: 'collapse'}}>
            <thead>
              <tr style={{background: '#FAFBFC', borderBottom: '1px solid #F1F5F9'}}>
                ...
                <th style={thStyle}>Nom de la famille <SortIcon /></th>
                <th style={thStyle}>Type <SortIcon /></th>
                <th style={thStyle}>Nombre d'éléments</th>
                <th style={thStyle}>Créé le <SortIcon /></th>
            ...
            <tbody>
              {pageRows.length === 0 && (
                <tr>
                  <td colSpan={9} style={{padding: 60, textAlign: 'center', color: '#94A3B8', fontSize: '0.85rem'}}>
                    <i className='pi pi-inbox' style={{fontSize: '1.6rem', color: '#CBD5E1', display: 'block', marginBottom: 8}}></i>
                    Aucune famille pour cette catégorie
                  </td>
                </tr>
              )}
              {pageRows.map((row) => {
                ...
                return (
                  <tr key={row.id} style={{borderBottom: '1px solid #F8FAFC', transition: 'background 0.12s'}}
                      ...
                      data-testid={`famille-row-${row.id}`}>
                    <td style={tdStyle}>
                      <KebabMenu row={row} onDetail={onDetail} onDelete={onDelete} />
                    </td>
                    <td style={tdStyle}>
                    ...
                    <td style={tdStyle}>
                      <div style={{width: 36, height: 36, borderRadius: 8, border: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FFF'}}>
                        <i className={row.icon || 'pi pi-tag'} style={{color: '#475569', fontSize: '0.95rem'}}></i>
                      </div>
                    </td>
                    <td style={{...tdStyle, fontWeight: 600, color: '#0F172A'}}>{row.label || '—'}</td>
                    <td style={tdStyle}>
                      <span style={{display: 'inline-flex', padding: '4px 12px', borderRadius: 8, background: '#EFF6FF', color: '#1D4ED8', fontSize: '0.74rem', fontWeight: 700}}>
                        {tabLabel(getTabCode())}
                      </span>
                    </td>
                    <td style={{...tdStyle, color: '#475569'}}>{row.nbre ?? row.count ?? '—'}</td>
                    <td style={{...tdStyle, color: '#64748B'}}>{row.dateCreation || row.createdAt || '—'}</td>
                    <td style={tdStyle}>
                      <span style={{
                        display: 'inline-flex', alignItems: 'center', gap: 5,
                        padding: '4px 11px', borderRadius: 999, fontSize: '0.72rem', fontWeight: 700,
          ...
          <div style={{display: 'flex', alignItems: 'center', gap: 6}}>
            ...
            {Array.from({length: Math.min(totalPages, 5)}, (_, i) => i + 1).map((p) => (
              <button key={p} onClick={() => setPage(p)} style={{
                ...
                background: p === page ? 'linear-gradient(135deg, #2563EB 0%, #1D4ED8 100%)' : '#FFF',
...
const thStyle = {
  ...
  fontSize: '0.74rem', fontWeight: 700, color: '#64748B',
  textTransform: 'none', letterSpacing: '0',
}
const tdStyle = {padding: '16px', verticalAlign: 'middle'}

const SortIcon = () => (
  <span style={{marginLeft: 4, color: '#CBD5E1', fontSize: '0.65rem'}}>
