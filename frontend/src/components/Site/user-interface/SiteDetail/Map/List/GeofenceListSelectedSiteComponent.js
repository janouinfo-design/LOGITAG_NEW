...
import {
  ...
  getSelectedGeofenceIdsSelectedSite,
  removeGeoSite,
  removeGeofencing,
  setGeofencesSelectedSite,
...
import {
  getSelectedGeoClientSelectedSite,
  getSelectedSiteClient,
} from '../../../../../../store/slices/customer.slice'
import {getAddressesSelectedSite} from '../../../../slice/addressSite.slice'
const GeofenceListSelectedSiteComponent = (props) => {
  const list = useAppSelector(getGeofencesSelectedSite)
  let hash = useAppSelector(getHashSelectedSite)
  let selectedGeo = useAppSelector(getSelectedGeoClientSelectedSite)

  let geoWorkSite = useAppSelector(getGeoSite)
  const toast = useRef(null)

  const selectedIds = useAppSelector(getSelectedGeofenceIdsSelectedSite)
  ...
  const onSelect = (val, id) => {
    if (val) dispatch(setSelectedGeofenceIdsSelectedSite([...selectedIds, id]))
    else dispatch(setSelectedGeofenceIdsSelectedSite(selectedIds?.filter((v) => v != id)))
  }
  ...
  const toggleSelectAll = (e) => {
    ...
    if (e.checked) dispatch(setSelectedGeofenceIdsSelectedSite(list.map((o) => o.id)))
    else dispatch(setSelectedGeofenceIdsSelectedSite([]))
  }
  ...
  const onLayerClicked = (o) => {
    ...
    if (o.idnavixy !== null && o.type === 'polygon') {
      ...
      dispatch(fetchPointsGeo(+o.idnavixy)).then((res) => {
        ...
        dispatch(setGeoSiteSelectedSite(obj))
      })
      ...
      let check = turf.booleanPointInPolygon(point, poly)
      checkCondition = check
      dispatch(setGeoSiteSelectedSite(o))
      return checkCondition
    }
  }
  ...
  const removeGeofences = () => {
    setIsLoading(true)
    dispatch(removeGeofencing(selectedSite?.id)).then((res) => {
      if (res.meta.requestStatus === 'fulfilled') {
        // dispatch(fetchGeoForSite(selectedSite?.id))
        setIsLoading(false)
      }
  ...
  useEffect(() => {
    ...
    dispatch(setSelectedGeofenceIdsSelectedSite(selectedIds?.filter((k) => ids?.includes(k))))
  }, [])
...
export default GeofenceListSelectedSiteComponent
