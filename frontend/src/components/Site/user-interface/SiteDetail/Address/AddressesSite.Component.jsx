...
import {setAddressDetail, setEditAddress, setSelectedAddress} from '../../../slice/addressSite.slice'
...
const AddressesSiteComponent = (props) => {
  ...
  return (
    <div data-testid={`address-site-card-${id}`} style={{background: '#FFF', border: '1px solid #E2E8F0', borderRadius: 14, padding: 18, display: 'flex', gap: 16, alignItems: 'flex-start', width: '100%', boxShadow: '0 1px 2px rgba(15, 23, 42, 0.04)'}}>
      ...
      <div style={{display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 12, flexShrink: 0}}>
        ...
        <div style={{display: 'flex', gap: 6}}>
          <button title='Voir' onClick={openEdit} data-testid={`address-site-view-${id}`} style={{width: 34, height: 34, borderRadius: 8, border: '1px solid #E2E8F0', background: '#FFF', color: '#64748B', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center'}}><i className='pi pi-eye'></i></button>
          <button title='Modifier' onClick={openEdit} data-testid={`address-site-edit-${id}`} style={{width: 34, height: 34, borderRadius: 8, border: '1px solid #E2E8F0', background: '#FFF', color: '#64748B', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center'}}><i className='pi pi-pencil'></i></button>
          <button title='Supprimer' data-testid={`address-site-delete-${id}`} style={{width: 34, height: 34, borderRadius: 8, border: '1px solid #FECACA', background: '#FEF2F2', color: '#EF4444', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center'}}><i className='pi pi-trash'></i></button>
        </div>
      </div>
    </div>
  )
...
export default AddressesSiteComponent
